...
\newpage
\includegraphics[max size={\textwidth}{\textheight}]{ADIN Inspector Client-Server-Communication-login.png}
...
\item{getCollectionInRange(collection, key, start, end)}
\\
syntax: \{"cmd": "GETCOLLRANGE", "par": "<collection>", "key": "<keyvalue>", "start": "<startvalue>", "end:", "<endvalue>"\} \\
where <key> is the name of a key in the given collection and <startvalue> and <endvalue> are valid values for this key\\
expected reply: data set

\item{getCollectionInRangeSize(collection, key, start, end)}
\\
syntax: \{"cmd": "GETCOLLRANGESIZE", "key": "<keyvalue>", "start": "<startvalue>", "end:", "<endvalue>"\} \\
expected reply: collection size
...
\begin{itemize}
		...
		all following data base access.
		      \item[-]UserSession
			    ...
			    \\returns: void
		            ...
		            \\Instantiate a new UserSession and log in into the database  using the given credentials.
		      ...
		      \item[-]getCollectionInRange
		            \\parameters: String key - the parameter used for filtering
	...
	\item[•]ClientRequestHandler
		...
		The requested action are typically executed by calls to the database session object.
		      \item[-]handleRequest
