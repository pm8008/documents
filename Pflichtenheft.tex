...
These are ~\ref{fig:Diagram types}:

\textbf{Line chart}: to easily compare ,linearly, two variables from a data stream.
\\
\textbf{Network Diagram}: provides a simple way to visualize the network topology. This diagram also gives the user the ability to inspect the nodes of the network and select data from a data point to be used as a filter in other diagrams (refer to last section for usage example).
\textbf{Raster spike diagram}: this is useful to show the transmission window of each packet and it's corresponding partner.

\textbf{Scatter plot}: a way to visualize up to 4 dimensions of the data stream.
