...
\newglossaryentry{diagram}
{
	name=diagram,
	plural=diagrams,
	description={A graphical representation of \glspl{data point} as per a certain \gls{diagram type}. Each data point is represented by a symbol whose position, size, color, and shape represent values of the data point. The diagram has labeled axes and, for other properties of the symbol, a legend that allow the user to determine the values represented by each symbol}
}
...
\subsection{Software}
\item{Client}
	www-browser of the latet generation.
...
Server:
	...
	System capable of running all the server-software
