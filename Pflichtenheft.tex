...
\begin{description}
  \item{
	\begin{itemize}
		\item Datetime: both types have this
		\item 2 types of events: one is notification (warning etc.), one is actual data point
			\begin{itemize}
				...
				\begin{itemize}
					\item MAC source
					\item MAC destination
					\item Event type, eg. protocol or network layer
				\end{itemize}
			\end{itemize}
	\end{itemize}
   }
...
\begin{description}
    \item
	Raw packet metadata include Packet number, Time, Source, Destination, Protocol and Packet Length.
\end{description}
...
\begin{description}
  ...
  \item[NF400]
  The framework should be able to handle data streamed from at least 500 nodes.
